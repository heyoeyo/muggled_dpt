...
def convert_midas_state_dict_keys(config_dict: dict, midas_state_dict: dict) -> dict:
    """
...
def _convert_patch_embed_keys(key: str, weight_or_bias: str) -> str | None:
    """
...
def _convert_logit_scaling_tensors(key: str, key_tensor: torch.Tensor) -> torch.Tensor:
    """
...
def _convert_qv_bias_tensors(key: str, key_tensor: torch.Tensor, heads_per_stage: list[int]) -> torch.Tensor:
    """
...
def _convert_imgenc_keys(key: str, layers_per_stage: list[int]) -> str | None:
    """
    ...
    if has_prefix(key, block_prefix):
        ...
        new_prefix = f"stages.{stage_idx}.blocks.{block_idx}"
    ...
    if has_prefix(key, patch_merge_prefix):
        ...
        new_key = f"patch_merge_layers.{merge_layer_idx}.{last_2_suffix}"
        return new_key
...
def _convert_reassembly_keys(key: str, weight_or_bias: str) -> str | None:
    """
    ...
    if key.startswith(postproc_prefix):
        ...
        new_key = f"{new_prefix}.{new_seq_str}.{weight_or_bias}"
        return new_key
    ...
    if has_prefix(key, layer_rn_prefix):
        ...
        new_key = f"{new_prefix}.fuse_proj.{weight_or_bias}"
        return new_key
...
def _convert_fusion_keys(key: str, weight_or_bias: str) -> str | None:
    """
    ...
    if key.startswith(refinenet_prefix):
        ...
        if is_out_conv:
            new_key = f"{block_prefix}.{new_seq_str}.{weight_or_bias}"
            return new_key
        ...
        new_key = f"{block_prefix}.{new_seq_str}.{new_conv_str}.{weight_or_bias}"
        return new_key
...
def _convert_head_keys(key: str, weight_or_bias: str) -> str | None:
    """
    ...
    if key.startswith(output_head_prefix):
        ...
        new_key = f"{new_prefix}.{weight_or_bias}"
        return new_key
