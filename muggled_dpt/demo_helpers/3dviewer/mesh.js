...
class MeshData {
	...
	draw_elements = (draw_mode) => {
		/* Bind buffer data (e.g vertex & face indexing data) and draw using current gl program */

		// For convenience
		const gl = this._gl;

		// Bind vertex buffer & data to gl program
		...
		const attr_size = this._vert_dimensions;
		...
		// Do actual drawing
		const offset = 0;
		gl.drawElements(draw_mode, this._num_elements, WebGL2RenderingContext.UNSIGNED_INT, offset);
	...
	_bind_mesh_data = (vertex_xys, face_indexing_list, vertex_attr_location) => {
		...
		*/
		...
		// Specify how attribute data (xy coords) is formatted within the buffer for reading in vertex shader
		this._vert_dimensions = vertex_xys[0].length;
		const attr_size = this._vert_dimensions;
