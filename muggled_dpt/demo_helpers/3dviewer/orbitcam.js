...
class Orbit_Camera {
	...
	*/
	...
	constructor(initial_distance = 1, world_up = [0, 1, 0], world_right = [1, 0, 0]) {
		...
		this.camvecs = {up: null, right: null, position_norm: null, origin: null, origin_offset: null};
		this.reset();
	...
	reset = () => {
		...
		this.camvecs.origin_offset = [0, 0, 0];
		this.camvecs.up = [...this.worldvecs.up];
		this.camvecs.right = [...this.worldvecs.right];

		// Reset camera positioning
		const real_origin = VEC3.add(this.camvecs.origin, this.camvecs.origin_offset);
		const camforward = VEC3.norm_cross(this.camvecs.up, this.camvecs.right);
		this.camvecs.position_norm = VEC3.subtract(real_origin, camforward);
	...
	set_origin_offset = (camera_offset, tilt_rad) => {
		this.camvecs.origin_offset[1] = -camera_offset * Math.sin(tilt_rad);
		this.camvecs.origin_offset[2] = -camera_offset * Math.cos(tilt_rad);
	}
    ...
    get_world_to_view_mat4 = () => {
		...
		const world_xyz = VEC3.add(scaled_xyz, real_origin);

		const lookat_mat = MAT4.look_at(world_xyz, real_origin, this.camvecs.up);
    ...
    get_view_to_clipspace_mat4 = (fov_rad, display_ar = 1.0, use_orthographic_camera = false) => {
		...
		if (use_orthographic_camera) {
			...
			const real_origin = VEC3.add(this.camvecs.origin, this.camvecs.origin_offset);
			const movement_zoom = real_origin[2];
			const zoom = (this.distance + movement_zoom * 0.5) * 0.5;
...
class Orbit_Canvas {
    ...
    */
    ...
    _on_move = (event) => {
		...
		if (is_shift) {
			const x_shift = event.movementX * this.orbitcam.distance / 10.0;
			const y_shift = event.movementY * this.orbitcam.distance / 10.0;
			this.orbitcam.translate(x_shift, y_shift, 0);
		} else {
