...
def print_config_feedback(
    ...
    print(
        "",
        f"Config ({model_name}):",
        f"  Device: {device_str} ({nice_dtype_str})",
        f"  Cache: {using_cache}",
        f"  Resolution: {model_img_w} x {model_img_h}",
        sep="\n",
        flush=True,
    )
...
def reduce_overthreading(device_str: str) -> None:
    """
...
def get_total_cuda_vram_usage_mb() -> int:
