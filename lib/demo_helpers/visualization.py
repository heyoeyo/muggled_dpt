...
class DisplayWindow:
    ...
    def waitKey(self, frame_delay_ms: int = 1) -> tuple[bool, int]:
        """
...
class WindowTrackbar:
    ...
    def __init__(self, window_name: str, trackbar_name: str, max_value: int, initial_value: int = 0):
    ...
    def write(self, new_value: int) -> None:
        ...
        cv2.setTrackbarPos(self.name, self._window_name, safe_value)
        return
...
def grid_stack_by_columns_first(image_list: list[np.ndarray], num_columns: int) -> np.ndarray:
    """
