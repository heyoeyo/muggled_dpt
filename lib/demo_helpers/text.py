...
class TextDrawer:
    ...
    def xy_px(
        ...
        if self._bg_color is not None:
            image = cv2.putText(
                image, text, xy_px, self._font, self._scale, self._bg_color, self._bg_thick, self._ltype
            )
    ...
    def xy_centered(self, image: ndarray, text: str, color: tuple[int, int, int] = (255, 255, 255)) -> ndarray:
        """Helper used to draw x/y centered text"""
        return self.xy_norm(image, text, (0.5, 0.5), color)

    def adjust_scale(self, new_scale: float):
        self._scale = new_scale
        return self

    def check_will_fit_width(self, text: str, target_width: int, shrink_factor: float = 0.9) -> bool:
    ...
    def check_will_fit_height(self, text: str, target_height: int, shrink_factor: float = 0.9) -> bool:
        """Helper used to check if text could be written into given container height"""
        txt_w, _, _ = self.get_text_size(text)
        return txt_w < int(target_height * shrink_factor)

    def get_text_size(self, text: str) -> tuple[int, int, int]:
