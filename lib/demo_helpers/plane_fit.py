...
def get_xyz_samples(
    depth_data: ndarray,
    num_x_samples: int = 16,
    num_y_samples: int = 16,
    jitter_scale: float = 0.75,
...
def find_plane_normal(xyz_samples: ndarray, xyz_mean: ndarray | None = None) -> ndarray:
    """
    ...
    if bad_shape:
        raise TypeError(f"Expecting samples of shape: [num samples, 3] Got: {shape}")
    not_xyz = shape[1] != 3
    if not_xyz:
        raise TypeError(f"Expecting columns of 'x,y,z', so shape should be [num samples, 3]. Got: {shape}")
...
def generate_image_from_plane_normal(
    output_shape_hw: tuple[int, int],
    plane_normal: tuple[float, float, float] | ndarray,
    xyz_mean: tuple[float, float, float] | ndarray | None = None,
) -> ndarray:
    """
    ...
    # For convenience
    num_rows, num_cols = output_shape_hw[0:2]
...
def estimate_plane_of_best_fit(depth_data: ndarray, samples_per_side: int = 16) -> ndarray:
    """
    ...
    plane_image = generate_image_from_plane_normal(depth_data.shape, plane_normal_vector, xyz_mean)
