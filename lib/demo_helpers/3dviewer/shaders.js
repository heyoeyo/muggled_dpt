...
function make_vertex_shader(is_metric_depth = false) {
	...
	#define adjust_factor       u_scaling_factors.z
	...
	float tan_angle = tan(half_fov_rad);
	float adj_depth = depth_value + adjust_factor;
	float x_world = adj_depth * vert_xy.x * x_scale * tan_angle;
	float y_world = adj_depth * vert_xy.y * y_scale * tan_angle;
...
function run_vertex_shader_cpu(vertex_xy_list, shader_uniforms, depth_image_data, is_metric_depth=false) {
	...
	*/
	...
	const [x_scale, y_scale, adjust_factor] = shader_uniforms.scaling_factors;
	...
	const tan_angle = Math.tan(half_fov_rad);
	...
	for(const [vx, vy] of vertex_xy_list) {
		...
		const adj_depth = depth_value + adjust_factor;
		const x_world = adj_depth * vx * x_scale * tan_angle;
		const y_world = adj_depth * vy * y_scale * tan_angle;
