...
async function create_glb_binary(vertex_xyz_list, vertex_uv_list, face_indexing_list, image_data, image_mimetype="image/png") {
...
async function create_glb_data(gldata, texturedata, meshdata, is_metric_depth=false, image_mimetype="image/png") {
  /* Helper used to generate glb save data from 3d viewer state data sets */
  ...
  const binary_data = await create_glb_binary(vert_xyz, vert_uv, face_indexing_list, image_data, image_mimetype);

  return [binary_data, image_data, depth_data];
}
