...
class Orbit_Camera {
	...
	*/
	...
	reset = () => {
		...
		// Reset camera positioning, based on up/right directions
		...
		this.distance = this._initial_distance;
	...
	translate = (dx, dy) => {
		...
		*/
		...
		const translation_xyz = VEC3.add(delta_up, delta_right);
...
class Orbit_Canvas {
    ...
    */
    ...
    _on_move = (event) => {
		...
		if (is_shift) {
			this.orbitcam.translate(event.movementX, event.movementY);
		} else {
