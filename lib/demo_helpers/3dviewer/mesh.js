...
class MeshData {
	...
	constructor(webgl_context, shaderdefs_attrs) {
		...
		this._vert_xys = [];
		this._face_indexing = [];
		this._mask_image = null;
		this._mask_image_channels = 0;
	...
	update = (
		mesh_wh, target_num_faces, vertex_jitter_pct, mask_threshold_pct, draw_mode=WebGL2RenderingContext.TRIANGLES,
	) => {
		...
		if (this._mask_image != null && mask_threshold_pct > 0) {
			[vert_xys, face_list] = this._apply_mask(vert_xys, face_list, mask_threshold_pct);
		}
	...
	_apply_mask = (vertex_xys_list, face_indexing_list, mask_threshold_pct) => {
		/* Function used to eliminate vertices/faces from mesh data, based on the current mask image */

		// Return inputs unchanged if we don't actually have a mask image!
		if (this._mask_image === null) {
			return [vertex_xys_list, face_indexing_list];
		}

		// For convenience
		const mask_image = this._mask_image;
		const num_channels = this._mask_image_channels;
		const mask_threshold = Math.max(0, Math.min(mask_threshold_pct * 255, 254));
		const xscale = (mask_image.width - 1.0) * 0.5;
		const yscale = (mask_image.height - 1.0) * 0.5;
		const num_verts = vertex_xys_list.length;

		// Loop over all vertices and record whether they land inside or outside of the given mask image
		const is_ok_vert_array = new Uint8Array(num_verts);
		let num_ok_verts = 0;
		let vx, vy, x_px, y_px, pixel_idx, is_in_mask;
		for(let i=0; i<num_verts; i++) {

			// Map +/-1 vertex coords to pixel indexing
			[vx, vy] = vertex_xys_list[i];
			x_px = Math.round((vx + 1.0) * xscale);
			y_px = Math.round((vy + 1.0) * yscale);

			// Record whether the vertex is in or out of the mask
			pixel_idx = (y_px * mask_image.width + x_px) * num_channels;
			is_in_mask = mask_image.data[pixel_idx] > mask_threshold;
			is_ok_vert_array[i] = is_in_mask;
			num_ok_verts += is_in_mask;
		}

		// Generate re-indexed vertices (i.e. account for gaps introduced by deleting vertices)
		const old_to_new_idx_lut = new Map();
		const new_vert_list = Array(num_ok_verts);
		let next_vert_idx = 0;
		for(let i=0; i<num_verts; i++) {
			if(is_ok_vert_array[i]) {
				new_vert_list[next_vert_idx] = vertex_xys_list[i];
		...
		for(let face of face_indexing_list) {
			...
			for(let vert_idx of face) {
				is_ok_face = is_ok_vert_array[vert_idx];
				if (!is_ok_face) break;
			}
			if (is_ok_face) {
		...
		return [new_vert_list, new_faces_list];
	}
	...
	store_mask_image = (image_data, num_channels=4) => {
		this._mask_image = image_data;
		this._mask_image_channels = num_channels;
		return this;
	}
