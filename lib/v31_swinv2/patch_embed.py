...
class PatchEmbed(nn.Module):
    ...
    Adapted for use with SwinV2, which includes layernorm
    Purpose is to take input images and convert them to 'lists' of (1D) tokens,
    one token for each (4x4 default) image patch.
    '''
    ...
    def __init__(self, features_per_token, patch_size_px=4, default_image_size=256, num_input_channels=3, bias=True):
        ...
        # -> Default size corresponds to the size of images used in training (e.g. 256 vs 384)
        # -> Tiling size is a constraint on the image input sizing, based on the need for
        #    having a whole number of patches (for patch embedding)
        # -> The SwinV2 models also include 3 downscaling (by a factor of 2) stages internally,
        #    so the number of patches must be divisible by 2^3 (8)
        self._default_size_px = round(default_image_size)
        self._tiling_size = round(8 * patch_size_px)
