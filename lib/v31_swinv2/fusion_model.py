...
class FusionModel(nn.Module):
    ...
    def __init__(self, num_fusion_channels: int, num_blocks: int = 4):
    ...
    def forward(
        self,
        noscale_featuremap: Tensor,
        downx2_featuremap: Tensor,
        downx4_featuremap: Tensor,
        downx8_featuremap: Tensor,
    ) -> Tensor:

        # Fuse all results together
        fuse_3 = self.blocks[3](downx8_featuremap)
        fuse_2 = self.blocks[2](downx4_featuremap, fuse_3)
        fuse_1 = self.blocks[1](downx2_featuremap, fuse_2)
...
class TopMostFusionBlock(nn.Module):
    ...
    def forward(self, imagelike_feature_map: Tensor) -> Tensor:
...
class FusionBlock(nn.Module):
    ...
    def forward(self, reassembly_feature_map: Tensor, previous_fusion_feature_map: Tensor) -> Tensor:
