...
def convert_state_dict_keys(config_dict: dict, midas_state_dict: dict) -> dict:
    """
...
def _convert_imgenc_keys(key: str, blocks_per_stage: int) -> str | None:
    """
    ...
    if has_prefix(key, block_prefix):
        ...
        new_prefix = f"stages.{stage_idx}.blocks.{new_block_idx}"
...
def _convert_reassembly_keys(key: str, weight_or_bias: str) -> str | None:
    ...
    if key.startswith(projection_prefix):
        ...
        new_key = f"{new_prefix}.resample.0.{weight_or_bias}"
        return new_key
    ...
    if key.startswith(resize_prefix):
        stage_idx = get_nth_integer(key, 0)
        new_prefix = stage_to_name_list[stage_idx]
        new_key = f"{new_prefix}.resample.1.{weight_or_bias}"
        return new_key
...
def _split_posenc_tensor(state_dict: dict) -> dict:
    """
