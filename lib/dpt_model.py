...
class DPTModel(nn.Module):
    ...
    This is a generic wrapper of the DPT model. To see more specific details
    (e.g. the implementation details of each model component) see the model
    specific 'maker' scripts, for example: make_depthanythingv2_dpt.py
    """
    ...
    def verify_input(self, image_rgb_normalized_bchw: Tensor) -> bool:
        ...
        assert ok_device, f"Device mismatch! Image: {img_device}, model: {model_device}"
        img_dtype = image_rgb_normalized_bchw.dtype
        model_dtype = next(self.parameters()).dtype
        ok_dtype = img_dtype == model_dtype
        assert ok_dtype, f"Data type mismatch! Image: {img_dtype}, model: {model_dtype}"
        ...
        assert ok_shape, f"Bad image shape! Image ({shape_str}) should have a shape of BxCXHxW"
